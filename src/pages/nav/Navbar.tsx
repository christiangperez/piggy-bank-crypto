...
const Navbar = (props: any) => {
	...
	const positionInLinks = NavbarLinks.filter(
		(p) => p.private === (account?.length > 0 && activeDeposit !== null) || p.private === false
			...
			<AppBar position="static" sx={{ background: 'primary' }}>
				<Toolbar>
					...
					{isMatch ? (
						<>
							<DrawerNav />
							<Typography sx={{ fontSize: '1.2rem', marginRight: 'auto' }}>Crypto Piggy Bank</Typography>
						</>
					) : (
						<>
							<IconButton edge="start" color="inherit" onClick={handleClickHome}>
								<SavingsIcon />
							</IconButton>
							<Tabs
					...
					{account ? (
						<IconButton edge="start" color="inherit" onClick={handleClickWallet}>
