      ...
      try {
        ...
        if (Number(amountInWei) >= Number(minimum)) {
          ...
          dispatch({
            ...
            payload: {
              okStatus: false,
              description: `The value must be greater than the minimum`,
            },
          });
        }
      } catch (error) {
        dispatch({
          ...
          payload: {
            okStatus: false,
            description: 'Exception error: ' + error,
          },
        });
      }
    } else {
...
export const startViewMyDeposit = () => {
  return async (dispatch: Dispatch<WalletTypes>, getState: any) => {
    ...
    if (contract) {
      if (account) {
        try {
          const myDeposit = await contract.methods
            .viewMyDeposit()
            .call({ from: account });
          ...
          dispatch({
            ...
            payload: {
              okStatus: false,
              description: 'An error has ocurred. ' + error,
            },
          });
        }
      } else {
        ...
        if (amount > 0) {
          try {
            ...
            dispatch({
              ...
              payload: {
                okStatus: true,
                description: 'Added deposit successfully!',
              },
            });
          } catch (e: any) {
            dispatch({
              ...
              payload: {
                okStatus: false,
                description: 'An error has ocurred. ' + e.message,
              },
            });
          }
        } else {
          dispatch({
            ...
            payload: {
              okStatus: false,
              description: 'You need to specifie an amount',
            },
          });
        }
      } else {
