					...
					.then(async (accounts: any) => {
						if (accounts.length > 0) {
							...
							dispatch({
								...
								payload: { okStatus: true, description: 'Wallet connected' },
							});
						}
					})
					.catch((err: any) => {
						...
						if (err.code === 4001) {
							...
							dispatch({
								type: 'showSnackbarTransactionResult',
								payload: { okStatus: false, description: 'Error. Reconnect your Metamask' },
							});
						} else {
							console.error(err);
						}
			...
			try {
				...
				if (Number(amountInWei) >= Number(minimum)) {
					...
					if (daysToExpire >= minimumDays) {
						...
						dispatch({
							type: 'showSnackbarTransactionResult',
							payload: { okStatus: true, description: 'Piggy Bank created successfully!' },
						});
					} else {
						dispatch({
							type: 'showSnackbarTransactionResult',
							payload: {
								okStatus: false,
								description: `You need to choose a expire days bigger than ${minimumDays} days`,
							},
						});
					}
				} else {
					dispatch({
						type: 'showSnackbarTransactionResult',
						payload: { okStatus: false, description: `The value must be greater than the minimum` },
					});
				}
			} catch (error) {
				dispatch({
					type: 'showSnackbarTransactionResult',
					payload: { okStatus: false, description: 'Exception error: ' + error },
				});
			}
		} else {
			dispatch({
				type: 'showSnackbarTransactionResult',
				payload: { okStatus: false, description: 'You need to connect your Wallet' },
			});
		}
	};
};
		...
		if (contract) {
			if (account) {
				try {
					...
					dispatch({
						type: 'showSnackbarTransactionResult',
						payload: { okStatus: false, description: 'An error has ocurred. ' + error },
					});
				}
			} else {
			...
			if (account) {
				if (amount > 0) {
					try {
						...
						dispatch({
							type: 'showSnackbarTransactionResult',
							payload: { okStatus: true, description: 'Added deposit successfully!' },
						});
					} catch (e: any) {
						dispatch({
							type: 'showSnackbarTransactionResult',
							payload: { okStatus: false, description: 'An error has ocurred. ' + e.message },
						});
					}
				} else {
					dispatch({
						type: 'showSnackbarTransactionResult',
						payload: { okStatus: false, description: 'You need to specifie an amount' },
					});
				}
			} else {
				dispatch({
		...
		if (contract) {
			if (account) {
				try {
					...
					dispatch({
						type: 'showSnackbarTransactionResult',
						payload: { okStatus: true, description: 'Sucessful Release!' },
					});
				} catch (error) {
					dispatch({
						type: 'showSnackbarTransactionResult',
